...
exports.generatedValidTypeName = generatedValidTypeName;
 ...
 */
function generateFilePath(file, bootstrapWkt, filesToGenerate, mapImports) {
    ...
    const wktFrom = codegenv2_1.wktPublicImportPaths[file.proto.name];
    if (wktFrom !== undefined &&
        !bootstrapWkt &&
        !filesToGenerate.find((f) => f.name === file.name)) {
        return wktFrom;
    }
    const target = (0, map_imports_js_1.mapImportTarget)(file.proto.name, mapImports);
    if (target !== undefined) {
 ...
 */
function generatedValidTypeName(desc) {
...
function idealValidTypeName(desc, i) {
    const salt = i === 0 ? "" : i === 1 ? "$" : `$${i - 1}`;
    return (0, safe_identifier_js_1.safeIdentifier)(identifier(desc) + "Valid" + salt);
}
/**
